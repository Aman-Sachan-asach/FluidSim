#pragma once

#include "../external/vec.h"
#include <vector>
#include "globals.h" 

// GridData is capable of storing any data in a grid
// Columns are indexed with i and increase with x
// Rows    are indexed with j and increase with z
// Stacks  are indexed with k and increase with y

// GridData is initialized by global variables theDim and theCellSize defined in GridData.cpp.

// theDim defines the number of cells in each X,Y,Z direction.
// theCellSize defines the size of each cell.
// GridData's world space dimensions extend from (0,0,0) to mMax
// mMax is (theCellSize*theDim[0], theCellSize*theDim[1], theCellSize*theDim[2])
class GridData
{
public:
   GridData();
   GridData(const GridData& orig) : mDfltValue(orig.mDfltValue), mMax(orig.mMax), mData(orig.mData) {};
   virtual ~GridData() {};

   virtual GridData& operator=(const GridData& orig);
   virtual double& operator()(int i, int j, int k);
   virtual const double operator()(int i, int j, int k) const; 

   // Initialize underlying data structure with dlftValue
   virtual void initialize(double dfltValue = 0.0);

   // Given a point in world coordinates, return the corresponding value from this grid. 
   // mDfltValue is returned for points outside of our grid dimensions
   virtual double interpolate(const vec3& pt);
  
   double CINT(double q_i_minus_1, double q_i, double q_i_plus_1, double q_i_plus_2, double x) const;

   // Access underlying data structure (for use with other UBLAS objects)
   std::vector<double>& data();

   // Given a point in world coordinates, return the cell index (i,j,k) corresponding to it
   virtual void getCell(const vec3& pt, int& i, int& j, int& k);

protected:
   virtual vec3 worldToSelf(const vec3& pt) const;

protected:
   double mDfltValue;
   vec3 mMax;
   std::vector<double> mData;
};

class GridDataX : public GridData
{
public:
   GridDataX(): GridData() {};
   virtual ~GridDataX() {};
   virtual void initialize(double dfltValue = 0.0);
   virtual double& operator()(int i, int j, int k);
   virtual const double operator()(int i, int j, int k) const; 
   virtual vec3 worldToSelf(const vec3& pt) const;
};

class GridDataY : public GridData
{
public:
   GridDataY(): GridData() {};
   virtual ~GridDataY() {};

   virtual double& operator()(int i, int j, int k);
   virtual const double operator()(int i, int j, int k) const;

   virtual void initialize(double dfltValue = 0.0);
   virtual vec3 worldToSelf(const vec3& pt) const;
};

class GridDataZ : public GridData
{
public:
   GridDataZ(): GridData() {};
   virtual ~GridDataZ() {};

   virtual double& operator()(int i, int j, int k);
   virtual const double operator()(int i, int j, int k) const; 
   
   virtual void initialize(double dfltValue = 0.0);
   virtual vec3 worldToSelf(const vec3& pt) const;
};

struct GridDataMatrix 
{
   GridDataMatrix() 
   {
      diag.initialize();
      plusI.initialize();
      plusJ.initialize();
      plusK.initialize();
   }
   GridData diag;
   GridData plusI;
   GridData plusJ;
   GridData plusK;
};